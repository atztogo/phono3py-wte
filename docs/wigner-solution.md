...
To compute the Wigner conductivity with scattering in the RTA approximation, specify `--br` and `--tt wte`. For `example/Wigner_La2Zr2O7`, the command is:

```bash
phono3py --nac --cell POSCAR --fc2 --dim="2 2 2" --dim-fc2="4 4 4" --mesh="19 19 19" --tmin=300 --tmax=1000 --tstep=700 --fc-symmetry --isotope --br --tt wte --read-gamma > tc_La2Zr2O7.out
```
...
```bash
% phono3py-load --mesh 11 11 11 --ts 1600 --br --tt wte
```
and the output is
```bash
...
...
To compute the Wigner conductivity treating scattering exactly, specify `--lbte` and `--tt wte`. For `example/Si-PBEsol`, the command is:

```bash
% phono3py-load --mesh 11 11 11 --ts 1600 --lbte --tt wte
```
and the output is
```bash
...
...
Using the code with the `--tt wte` option has a negligible effect on the duration of the calculation.
